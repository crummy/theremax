...
export class Theremax {
	...
	moveDraw(x: number, y: number, pointerId: number) {
		const recording = this.recordings.find(
			(r) => r.pointerId === pointerId && r.activelyRecording,
		);
		if (!recording) {
	...
	getIntervals() {
		...
		return this.recordings
			.find((r) => r.activelyRecording)
			?.instrument.getIntervals();
	}
	...
	tick(): { [recordingId: number]: { x: number; y: number }[] } {
		...
		for (const [recordingId, recording] of Object.entries(this.recordings)) {
			...
			while (
				recording.lastPlayed?.next !== undefined &&
				recording.lastPlayed.millis <= now
			) {
				// deduplicate points
				if (
					recording.lastPlayed.next.x !== recording.lastPlayed.x ||
					recording.lastPlayed.next.y !== recording.lastPlayed?.y
				) {
			...
			if (recording.end.millis < now && !recording.activelyRecording) {
				...
				this.playScaled(
					recording.lastPlayed.x,
					recording.lastPlayed.y,
